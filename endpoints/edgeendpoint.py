...
def create_app():
    ...
    def process_image_multi_view():
        ...
        try:
            my_logger.info('process_image__full():: Image processing starting.')
            processor_response = process_img_request(str(request.remote_addr), request_json['prediction'], request_json['frame'], request_json['cam'], multi_view_chain.get_processor())
            my_logger.info('process_image__full():: Image processing completed.')
        except OSError:
