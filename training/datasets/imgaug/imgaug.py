...
def is_np_array(val):
    """
    Checks whether a variable is a numpysteputil array.
    ...
    -------
    out : bool
        True if the variable is a numpysteputil array. Otherwise False.

    """
    # using np.generic here seems to also fire for scalar numpysteputil values even
...
def is_integer_array(val):
    """
    Checks whether a variable is a numpysteputil integer array.
    ...
    -------
    out : bool
        True if the variable is a numpysteputil integer array. Otherwise False.
...
def is_float_array(val):
    """
    Checks whether a variable is a numpysteputil float array.
    ...
    -------
    out : bool
        True if the variable is a numpysteputil float array. Otherwise False.
...
def new_random_state(seed=None, fully_random=False):
    ...
    fully_random : bool, optional(default=False)
        Whether to use numpysteputil's random initialization for the
...
def copy_random_state(random_state, force_copy=False):
    ...
    force_copy : bool, optional(default=False)
        ...
        state. If False, it will not copy numpysteputil's default random state,
...
def quokka(size=None):
    """
    Returns an image of a quokka as a numpysteputil array.
...
def quokka_square(size=None):
    """
    Returns an (square) image of a quokka as a numpysteputil array.
...
def pad(arr, top=0, right=0, bottom=0, left=0, mode="constant", cval=0):
    ...
    This function is a wrapper around `numpysteputil.pad()`.
    ...
    mode : string, optional(default="constant")
        Padding mode to use. See `numpysteputil.pad()` for details.

    cval : number, optional(default=0)
        Value to use for padding if mode="constant". See `numpysteputil.pad()` for details.
