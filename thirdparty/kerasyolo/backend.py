...
from keras.layers import Reshape, Activation, Conv2D, Input, MaxPooling2D, BatchNormalization, Flatten, Dense, Lambda, Maximum
...
TINY_YOLO_BACKEND_PATH  = "yolov2tinyvoc_v1.h5"   # should be hosted on a server
...
class TinyYoloFeature(BaseFeatureExtractor):
    ...
    def __init__(self, input_size):
        ...
        self.feature_extractor = Model(input_image, x)
        #self.feature_extractor.load_weights('D:\\Files\\Box Sync\\research\\source code\\keras-yolo2\\yolov2tinyvoc_v4.h5')
...
class MulticamTinyYoloFeature(BaseFeatureExtractor):
    """docstring for ClassName"""
    def __init__(self, input_size):
        num_inputs = 3
        input_images = [Input(shape=(input_size, input_size, 3)) for _ in range(num_inputs)]
        ...
        if num_inputs < 2:
            ...
            fused_net = Maximum()([nets[index] for index in range(num_inputs)])
        ...
        self.feature_extractor = Model([input_images[0], input_images[1], input_images[2]], fused_net)

    def build_net(self, input_image, cam_index):
        # Layer 1
        x = Conv2D(16, (3, 3), strides=(1, 1), padding='same', name='conv_1' + '_' + str(cam_index), use_bias=False)(input_image)
        x = BatchNormalization(name='norm_1' + '_' + str(cam_index))(x)
        x = LeakyReLU(alpha=0.1)(x)
        x = MaxPooling2D(pool_size=(2, 2))(x)

        # Layer 2 - 5
        for i in range(0, 4):
            x = Conv2D(32 * (2 ** i), (3, 3), strides=(1, 1), padding='same', name='conv_' + str(i + 2) + '_' + str(cam_index),
                       use_bias=False)(x)
            x = BatchNormalization(name='norm_' + str(i + 2) + '_' + str(cam_index))(x)
            x = LeakyReLU(alpha=0.1)(x)
            x = MaxPooling2D(pool_size=(2, 2))(x)

        # Layer 6
        x = Conv2D(512, (3, 3), strides=(1, 1), padding='same', name='conv_6' + '_' + str(cam_index), use_bias=False)(x)
        x = BatchNormalization(name='norm_6' + '_' + str(cam_index))(x)
        x = LeakyReLU(alpha=0.1)(x)
        x = MaxPooling2D(pool_size=(2, 2), strides=(1, 1), padding='same')(x)

        # Layer 7 - 8
        for i in range(0, 2):
            x = Conv2D(1024, (3, 3), strides=(1, 1), padding='same', name='conv_' + str(i + 7) + '_' + str(cam_index), use_bias=False)(x)
            x = BatchNormalization(name='norm_' + str(i + 7) + '_' + str(cam_index))(x)
            x = LeakyReLU(alpha=0.1)(x)
