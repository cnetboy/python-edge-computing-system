...
class Augmenter(object): # pylint: disable=locally-disabled, unused-variable, line-too-long
    ...
    def augment_batches(self, batches, hooks=None, background=False):
        ...
        for i, batch in enumerate(batches):
            if isinstance(batch, ia.Batch):
                ...
                ia.do_assert(batch.ndim in (3, 4), "Expected numpysteputil array to have shape (N, H, W) or (N, H, W, C), got %s." % (batch.shape,))
                ...
                if len(batch) == 0:
                    ...
                    raise Exception("Unknown datatype in batch[0]. Expected numpysteputil array or imgaug.KeypointsOnImage, got %s." % (type(batch[0]),))
            else:
                raise Exception("Unknown datatype of batch. Expected imgaug.Batch or numpysteputil array or list of numpysteputil arrays/imgaug.KeypointsOnImage. Got %s." % (type(batch),))
    ...
    def augment_images(self, images, parents=None, hooks=None):
        ...
        images : (N,H,W,C) ndarray or (N,H,W) ndarray or list of (H,W,C) ndarray or list of (H,W) ndarray
            Images to augment. The input can be a list of numpysteputil arrays or
        ...
        if ia.is_np_array(images):
            ...
            if images_copy.ndim == 3 and images_copy.shape[-1] in [1, 3]:
                warnings.warn("You provided a numpysteputil array of shape %s as input to augment_images(), "
            ...
            raise Exception("Expected images as one numpysteputil array or list/tuple of numpysteputil arrays, got %s." % (type(images),))
...
class Sometimes(Augmenter):
    ...
    def _augment_images(self, images, random_state, parents, hooks):
        if hooks.is_propagating(images, augmenter=self, parents=parents, default=True):
            ...
            # otherwise it was a numpysteputil array, so make the output a numpysteputil array too.
